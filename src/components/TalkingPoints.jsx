...
import { talkingPoints } from '../data/talking-points'
...
const ShareButton = ({ slug }) => {
  ...
  const onClick = async () => {
    const url = `${window.location.origin}/talking-points#${slug}`
    try {
  ...
  return (
    <button
      ...
      aria-label='Copy share link to this talking point'
    >
...
const TalkingPointCard = ({ item, isHighlighted }) => (
  <li
    id={item.id}
...
const TalkingPoints = () => {
  ...
  return (
    <section className='container mx-auto mb-8' id='talking-points'>
      {/* Title */}
      ...
      <div className='px-4 md:px-8 mb-12 max-w-4xl space-y-4'>
        <p className='font-mono text-lg sm:text-xl lg:text-2xl leading-snug opacity-90'>
          ...
          common ones — and the talking points to fire back with.
        </p>
        <p className='font-mono text-sm sm:text-base lg:text-lg leading-snug opacity-70'>
          ...
          copy a link that scrolls directly to that talking point — drop it
          under someone’s reply and let the receipts do the talking.
        </p>
      </div>
      ...
      <div className='px-4 md:px-8 mt-16 pt-8 border-t-2 border-white max-w-4xl'>
        <p className='font-mono text-sm sm:text-base opacity-70 leading-snug'>
          Spotted another talking point that should be here? Send it to the
          team — we’ll add it to the list. The wording on existing entries is
          being actively polished with input from Lauren Rodriguez and Rafael
          Yakobi.
        </p>
      </div>
    </section>
